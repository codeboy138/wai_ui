...
const TimelinePanel = {
                    ...
                    <div id="timeline-lane-container" class="relative min-w-max" :style="{ minHeight: totalTrackHeight + 24 + 'px' }" @mousedown="handleLaneMouseDown" @mouseenter="isMouseInLane = true" @mouseleave="isMouseInLane = false" @wheel.prevent="handleLaneWheel">
                        ...
                        <div v-for="(track, idx) in vm.tracks" :key="track.id" :data-track-id="track.id" :data-track-index="idx" class="border-b border-ui-border relative track-lane" :class="{ 'opacity-30': track.isHidden }" :style="{ height: getTrackHeight(track.id) + 'px' }" @mousedown="onTrackLaneMouseDown($event, track)" @contextmenu.prevent="openClipContextMenu($event, track)">
                            ...
                            <div v-for="clip in getVisibleClipsForTrack(track.id)" :key="clip.id" :data-clip-id="clip.id" class="clip absolute cursor-pointer" :class="getClipClasses(clip)" :style="getClipStyle(clip, track)" @mousedown.stop="onClipMouseDown($event, clip, track)" @contextmenu.stop.prevent="openClipContextMenu($event, track, clip)">
                                ...
                                <div class="clip-resize-handle clip-resize-handle-left" @mousedown.stop="startClipResize($event, clip, 'left')"><i class="fa-solid fa-chevron-left"></i></div>
                                <div class="clip-resize-handle clip-resize-handle-right" @mousedown.stop="startClipResize($event, clip, 'right')"><i class="fa-solid fa-chevron-right"></i></div>
                            </div>
                        </div>
    ...
    methods: {
        ...
        getFilmstripFrames: function(clip) { 
            ...
            // 클립의 소스 오프셋 (트리밍된 경우)
            var startOffset = clip.startOffset || 0;
            var sourceDuration = clip.sourceDuration || clip.duration;
            
            for (var i = 0; i < frameCount; i++) { 
                // 클립 내 상대적 위치 (0~1)
                var clipRelativePosition = i / frameCount;
                // 소스 비디오에서의 실제 시간 위치
                var sourceTimePosition = startOffset + (clipRelativePosition * clip.duration);
                // 소스 전체 길이 대비 비율
                var sourceRelativePosition = sourceDuration > 0 ? sourceTimePosition / sourceDuration : clipRelativePosition;
                // 캐시된 썸네일 인덱스
                var thumbnailIndex = Math.floor(sourceRelativePosition * cachedCount);
                thumbnailIndex = Math.max(0, Math.min(cachedCount - 1, thumbnailIndex));
        ...
        // [수정됨] 드래그 중 원본 클립 숨기기 (visibility: hidden)
        ...
        selectClip: function(clipId, mod) { 
            ...
            if (mod.ctrlKey || mod.metaKey) { 
                ...
                var li = -1, ci = -1; 
                for (var i = 0; i < tc.length; i++) { if (tc[i].id === self.lastSelectedClipId) li = i; if (tc[i].id === clipId) ci = i; } 
        ...
        onClipMouseDown: function(e, clip, track) {
            ...
            var isAlreadySelected = this.selectedClipIds.indexOf(clip.id) >= 0;
            this.clickedOnSelectedClip = isAlreadySelected;
            
            if (!e.ctrlKey && !e.metaKey && !e.shiftKey && !isAlreadySelected) {
...
// 코드연결지점
        onDocumentMouseMove: function(e) {
            ...
            if (this.pendingClickClipId && !this.isDraggingClip && this.draggingClipIds.length > 0) {
                if (Math.abs(e.clientX - this.dragStartX) > 3 || Math.abs(e.clientY - this.dragStartY) > 3) {
                    ...
                    this.clickedOnSelectedClip = false;
        ...
        finalizeDrop: function(e) {
            ...
            this.draggingClipIds.forEach(function(id) {
                ...
                if (finalPosition !== null) {
                    clipUpdates.push({ clip: clip, start: finalPosition, trackId: targetTrackId });
                } else {
                    allSuccess = false;
                }
            });
            
            if (allSuccess && clipUpdates.length > 0) {
                clipUpdates.forEach(function(update) {
                    update.clip.start = update.start;
                    update.clip.trackId = update.trackId;
                });
                this.saveToHistory();
            } else {
        ...
        openClipContextMenu: function(e, track, clip) { 
            ...
            if (clip) {
                ...
                // 핵심 수정: 이미 선택된 클립 중 하나를 우클릭한 경우 -> 기존 선택 모두 유지
                if (isAlreadySelected) {
                    // 아무것도 하지 않음 - 기존 다중 선택 유지
                } else {
