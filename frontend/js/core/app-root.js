...
const AppRoot = {
    ...
    computed: {
        ...
        resolutionOptions() { return RESOLUTION_KEYS.map(key => this.getResolutionLabelFor(key)); },
        currentResolutionLabel() { return this.getResolutionLabelFor(this.resolution); },
        layerConfigBox() {
    ...
    watch: {
        canvasSize: { handler(newSize) { try { if (window.PreviewRenderer && typeof window.PreviewRenderer.setCanvasSize === 'function') { window.PreviewRenderer.setCanvasSize(newSize); } } catch (err) { console.warn('[Preview] canvasSize watcher failed:', err); } }, deep: true },
        canvasBoxes: { handler(newBoxes) { try { if (window.PreviewRenderer && typeof window.PreviewRenderer.syncBoxes === 'function') { window.PreviewRenderer.syncBoxes(newBoxes); } } catch (err) { console.warn('[Preview] canvasBoxes watcher failed:', err); } }, deep: true }
    },
    mounted() {
        this.$nextTick(() => { 
    ...
    beforeUnmount() { if (this._spinWheelHandler) { document.removeEventListener('wheel', this._spinWheelHandler); this._spinWheelHandler = null; } },
    methods: {
        ...
        firePython(f) { console.log('Py:', f); if (window.backend && window.backend[f]) { window.backend[f](); } else { console.log(`[DUMMY] Python call: ${f}`); } },
        toggleDevMode(mode) {
            if (mode === 'active') {
        ...
        copyInspectorId() {
            ...
            else { const ta = document.createElement('textarea'); ta.value = id.toString(); ta.style.position = 'fixed'; ta.style.left = '-9999px'; document.body.appendChild(ta); ta.select(); try { document.execCommand('copy'); } catch (err) { console.warn('[Inspector] execCommand copy failed', err); } document.body.removeChild(ta); }
        },
        ...
        setupInspectorMode() {
            ...
            document.addEventListener('mousemove', (e) => {
                ...
                let target = e.target;
                if (target.classList.contains('dev-highlight') || target.classList.contains('dev-tooltip')) { const realTarget = document.elementFromPoint(e.clientX, e.clientY); if (realTarget) target = realTarget; }
                if (target && target.tagName !== 'HTML' && target.tagName !== 'BODY') {
                    ...
                    let top = rect.top - 80 - 10, left = rect.left + rect.width + 10;
        ...
        buildDevInfo(targetEl) {
            ...
            const lines = [];
            if (spec) { if (spec.module) lines.push(`module: ${spec.module}`); if (spec.py_func) lines.push(`py: ${spec.py_func}`); if (spec.js_action) lines.push(`js: ${spec.js_action}`); } else { lines.push('Spec: NOT FOUND'); }
        ...
        setResolution(labelOrKey) { const str = (labelOrKey || '').toString().trim(); const match = str.match(/^(\S+)/); this.resolution = match ? match[1] : (str || this.resolution); this.updateCanvasSizeFromControls(); },
        ...
        getResolutionLabelFor(key) { const k = key || this.resolution || 'FHD'; const size = this.computeResolutionSize(this.aspectRatio, k); return `${k} (${size.w} x ${size.h})`; },
        recalculateCanvasSizeFromWrapper() {
            const wrapper = document.getElementById('preview-canvas-wrapper'); if (!wrapper) return;
            const PADDING = 20;
            const innerW = wrapper.clientWidth - PADDING * 2, innerH = wrapper.clientHeight - PADDING * 2;
            if (innerW <= 0 || innerH <= 0) return;
            const cw = this.canvasSize.w || 1920, ch = this.canvasSize.h || 1080;
            const cRatio = cw / ch, wrapperRatio = innerW / innerH;
        ...
        updateCanvasMouseCoord(e) {
            ...
            const wrapper = document.getElementById('preview-canvas-wrapper'), scaler = document.getElementById('preview-canvas-scaler'); if (!wrapper || !scaler) return;
            const wRect = wrapper.getBoundingClientRect(), sRect = scaler.getBoundingClientRect();
            const PADDING = 20;
            const mouseXInWrapper = e.clientX - wRect.left, mouseYInWrapper = e.clientY - wRect.top;
            ...
            const canvasX = e.clientX - sRect.left, canvasY = e.clientY - sRect.top;
        ...
        setupCanvasScaler() {
            ...
            updateLayout();
            if (window.ResizeObserver) new ResizeObserver(updateLayout).observe(wrapper); else window.addEventListener('resize', updateLayout);
        },
        setupSpinWheel() {
            const handler = (event) => {
                ...
                const min = target.min !== '' ? parseFloat(target.min) : 0, max = target.max !== '' ? parseFloat(target.max) : +Infinity;
        ...
        deleteLayerTemplate(templateId) {
            this.layerTemplates = this.layerTemplates
