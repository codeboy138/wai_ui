...
const VisualizationModal = {
    ...
    methods: {
        ...
        selectItem(e, item) {
            if (e.ctrlKey || e.metaKey) {
            ...
            } else if (e.shiftKey && this.selectedItemIds.length > 0) {
                const lastId = this.selectedItemIds[this.selectedItemIds.length - 1];
                const items = this.filteredItems;
                let lastIdx = -1, curIdx = -1;
                for (let i = 0; i < items.length; i++) {
                    if (items[i].id === lastId) lastIdx = i;
                    if (items[i].id === item.id) curIdx = i;
                }
                if (lastIdx >= 0 && curIdx >= 0) {
                    ...
                    for (let j = minIdx; j <= maxIdx; j++) {
                        this.selectedItemIds.push(items[j].id);
                    }
                }
            } else {
