...
const AppRoot = {
    ...
    watch: {
        ...
        // 클립 변경 감시 - 삭제된 클립의 박스도 정리
        clips: {
            handler(newClips) {
                this.cleanupOrphanedBoxes(newClips);
            },
            deep: true
        }
    },
    ...
    methods: {
        ...
        // === 클립-박스 매핑 초기화 ===
        initClipBoxMapping() {
            // 기존 클립에 대해 박스가 없으면 생성
            this.clips.forEach(clip => {
                if (clip.type === 'video' || clip.type === 'image') {
                    this.ensureBoxForClip(clip);
                }
            });
        },
        ...
        ensureBoxForClip(clip) {
            ...
            const newBox = {
                ...
                zIndex: zIndex,
                isHidden: true, // 초기에는 숨김 (시간에 따라 표시)
        ...
        removeBox(id) {
            ...
            if (box && box.clipId) {
                // clipBoxMap에서도 제거
                delete this.clipBoxMap[box.clipId];
            }
        ...
        addClipFromDrop(fileType, trackIndex, time, assetName, assetSrc) {
        ...
        updateClip(clipId, startChange, durationChange) {
            ...
            if (index !== -1) {
                ...
                if (boxId) {
                    const box = this.canvasBoxes.find(b => b.id === boxId);
                    if (box) {
                        box.clipStart = newStart;
                        box.clipDuration = newDuration;
                    }
                }
            }
        },
        moveClip(clipId, timeChange) {
            ...
            if (index !== -1) {
                const clip = this.clips[index];
                const newClips = [...this.clips];
                const newStart = Math.max(0, clip.start + timeChange);
                newClips[index] = { ...clip, start: newStart };
                this.clips = newClips;
            ...
            if (index !== -1) {
                ...
                if (boxId) {
                    ...
                    if (box) {
                        ...
                        const trackIndex = this.tracks.findIndex(t => t.id === newTrackId);
                        box.zIndex = (this.tracks.length - trackIndex) * 10 + 100;
                        if (track) {
                            box.color = track.color;
                        }
                    }
                }
            }
